...
export const TaskDetailModal: React.FC<TaskDetailModalProps> = ({ task, onClose, onUpdate }) => {
  ...
  const handleAddTag = () => {
    ...
    if (!color) return;
    const newTag: Tag = { id: Math.random().toString(36).substr(2, 9), text, color };
  ...
  const handleAddResource = () => {
    const name = prompt("🍓 資源名稱");
    if (!name) return;
    const url = prompt("🌐 貼上網址");
    if (!url) return;
    let category: ResourceCategory = 'link';
    const lowUrl = url.toLowerCase();
    if (lowUrl.match(/\.(jpeg|jpg|gif|png|webp)$/)) category = 'image';
    else if (lowUrl.includes('figma.com')) category = 'design';
    const newAttachment: Attachment = { id: Math.random().toString(36).substr(2, 9), name, url, category, createdAt: new Date().toISOString() };
  ...
  return (
    <div className="fixed inset-0 z-[60] flex justify-end bg-black/20 backdrop-blur-sm animate-in fade-in duration-300" onClick={onClose}>
      <div className="w-full max-w-xl bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-500 rounded-l-[40px] border-l-4 border-pink-100 overflow-hidden" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto p-8 space-y-10 no-scrollbar pb-24">
          ...
          <div className="space-y-2">
            ...
            <input value={task.title} onChange={(e) => onUpdate({ title: e.target.value })} className="text-2xl font-bold text-[#5c4b51] w-full bg-pink-50/20 border-2 border-transparent focus:border-pink-100 focus:bg-white focus:outline-none rounded-2xl px-3 py-2 transition-all" />
          </div>
            ...
            <div className="flex flex-wrap gap-2.5">
              {task.tags?.map(tag => (
                <div key={tag.id} className="group relative">
                  <span className="px-4 py-2 rounded-2xl text-[11px] font-black border flex items-center gap-2 shadow-sm transition-all pr-10" style={{ backgroundColor: tag.color + '22', borderColor: tag.color, color: tag.color }}>
                    <div className="w-2 h-2 rounded-full" style={{ backgroundColor: tag.color }} /> {tag.text}
                    <button onClick={() => removeTag(tag.id)} className="absolute right-3 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-opacity p-1 hover:bg-white/50 rounded-lg"><X size={12} /></button>
                  </span>
                </div>
              ))}
              <button onClick={handleAddTag} className="px-4 py-2 rounded-2xl text-[11px] font-black border border-dashed border-pink-200 text-pink-300 hover:border-pink-400 hover:text-pink-500 transition-all flex items-center gap-2"><Plus size={14} /> 新增標籤</button>
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="h-4 bg-pink-50 rounded-full overflow-hidden border border-pink-100 shadow-inner relative">
              <div className="h-full bg-pink-400 transition-all duration-300" style={{ width: `${task.progress}%` }} />
              <input type="range" min="0" max="100" value={task.progress} onChange={(e) => onUpdate({ progress: parseInt(e.target.value), status: parseInt(e.target.value) === 100 ? TaskStatus.COMPLETED : task.status })} className="absolute inset-0 w-full opacity-0 cursor-pointer z-10" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <select 
                ...
                className="w-full p-4 rounded-2xl border-2 border-white text-sm font-bold text-[#5c4b51] shadow-md cursor-pointer focus:ring-4 focus:ring-pink-100 transition-all"
                style={{ backgroundColor: COLORS.status[task.status] }}
              >
                {Object.values(TaskStatus).map(s => <option key={s} value={s} className="bg-white">{s}</option>)}
              </select>
            </div>
            <div className="space-y-2">
              ...
              >
                {Object.values(TaskPriority).map(p => <option key={p} value={p} className="bg-white">{p}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={handleAddResource} className="p-2 bg-pink-50 text-pink-500 rounded-xl hover:bg-pink-100 shadow-sm transition-all"><Plus size={18} /></button>
            </div>
              ...
              {(task.attachments || []).map(item => (
                <div key={item.id} className="group flex items-center gap-3 p-3 bg-pink-50/20 border border-pink-50 rounded-2xl hover:bg-white hover:shadow-md transition-all">
                  ...
                  <div className="flex items-center gap-2">
                    <a href={item.url} target="_blank" rel="noopener noreferrer" className="p-1.5 text-pink-400 hover:text-pink-600"><ExternalLink size={14} /></a>
                    <button onClick={() => onUpdate({ attachments: (task.attachments || []).filter(a => a.id !== item.id) })} className="opacity-0 group-hover:opacity-100 p-1.5 text-pink-200 hover:text-red-400 transition-opacity"><Trash2 size={14} /></button>
                  </div>
                </div>
              ))}
              {(task.attachments || []).length === 0 && <p className="text-center py-6 text-pink-200 text-xs italic">尚未新增資源連結 🍬</p>}
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              {!isEditingDesc ? (
                <button onClick={() => setIsEditingDesc(true)} className="flex items-center gap-2 px-5 py-2 rounded-xl bg-pink-50 text-pink-500 text-[10px] font-bold hover:bg-pink-100 transition-all"><Edit3 size={12} /> 編輯筆記</button>
              ) : (
                <div className="flex items-center gap-2 bg-pink-50 p-1 rounded-xl border border-pink-100">
                  ...
                  <button onClick={() => setShowPreviewDuringEdit(!showPreviewDuringEdit)} className="px-3 py-1 bg-white text-pink-500 rounded-lg text-[10px] font-bold shadow-sm">{showPreviewDuringEdit ? '編輯' : '預覽'}</button>
                  <button onClick={handleSaveDesc} className="px-3 py-1 bg-pink-500 text-white rounded-lg text-[10px] font-bold">儲存</button>
                </div>
              )}
            </div>
            <div className="w-full min-h-[250px] rounded-[32px] bg-white border border-pink-50 overflow-hidden shadow-inner flex flex-col">
              {isEditingDesc && !showPreviewDuringEdit ? (
                <textarea value={tempDesc} onChange={(e) => setTempDesc(e.target.value)} className="w-full flex-1 p-6 bg-transparent text-[#5c4b51] text-sm focus:outline-none resize-none font-mono leading-relaxed" autoFocus />
              ) : (
                <div className="p-6 prose prose-pink prose-sm flex-1 overflow-y-auto">
                  <ReactMarkdown remarkPlugins={[remarkGfm]}>{tempDesc || "*尚未輸入內容...*"}</ReactMarkdown>
                </div>
              )}
            </div>
            {isPreview ? (
