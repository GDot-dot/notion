      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto max-h-screen custom-scrollbar transition-all duration-300">
        {!isConfigured && (
          <div className="bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4 mb-6 rounded-r-xl flex items-center gap-3 animate-pulse">
            ...
            <p className="text-sm font-bold">⚠️ Firebase 尚未連線 🍓</p>
          </div>
        )}
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-6">
              ...
              <div className="flex items-center gap-2 mt-1 ml-2">
                <div className="flex items-center gap-2 px-2 py-0.5 bg-white/40 rounded-full border border-pink-100">
                  {state.isSyncing ? <><Loader2 size={12} className="text-pink-400 animate-spin" /><span className="text-[10px] text-pink-400 font-bold">同步中...</span></> : 
                    state.user ? <><CloudCheck size={12} className="text-green-400" /><span className="text-[10px] text-green-500 font-bold">雲端同步</span></> : 
                    <><Save size={12} className="text-blue-400" /><span className="text-[10px] text-blue-500 font-bold">本機儲存</span></>}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <button onClick={() => deleteProject(currentProject.id)} className="p-2.5 bg-white text-pink-300 hover:text-red-400 rounded-xl border border-pink-50 shadow-sm"><Trash2 size={20} /></button>
            ...
            <div className="space-y-8 md:space-y-12">
              ...
              <div className="bg-white rounded-[32px] md:rounded-[40px] p-6 md:p-8 cute-shadow border border-pink-100">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                  ...
                  <button onClick={addTask} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-pink-50 text-pink-500 px-6 py-2.5 rounded-2xl font-bold hover:bg-pink-100 shadow-sm"><Plus size={18} /> 新增任務</button>
                </div>
